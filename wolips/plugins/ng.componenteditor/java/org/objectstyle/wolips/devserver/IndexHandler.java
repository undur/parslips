	...
	private static final String INDEX = """
			{
			  ...
			  "endpoints": [
			    ...
			    {"path": "/launch", "params": "config|app, mode=debug|run, open=true?, ignoreErrors=true?, allowMultiple=true?, waitForPort?, timeout?, port?, args?, stopOthers=true?", "description": "Launch a config (preferring local/dev configs on project-name queries). Ports are decided explicitly: if the target port (the config's -WOPort, default 1200) is held, the launch is refused naming the holder; stopOthers=true stops the holder first (the usual choice in development), port=N runs alongside on another port (a -WOPort argument injected into an unsaved working copy, the saved config untouched; also the default waitForPort), args=... appends extra program arguments the same way. Refuses with a reason when the config is unknown, the project is closed (open=true overrides), the project OR ANY PROJECT IT DEPENDS ON has compile errors after an incremental build (ignoreErrors=true overrides; the refusal names the broken projects and the clean-rebuild call that usually fixes them) or it is already running (allowMultiple=true overrides). Never raises Eclipse's launch prompts (errors-in-workspace, save, switch-to-debug) - every decision is made here and reported as data. With waitForPort, blocks until the app answers, dies, a modal dialog appears (reported, see /dialogs), or timeout (default 60s). Omit config to list all configs."},
			    ...
			    {"path": "/console", "params": "app|config, tail?", "description": "The app's console output (tail, default 100 lines) — including after the process died, which is when you need it: startup failures are only visible here. The header carries state, exit code and end time, and notes when a sibling launch of the same project started just before this one ended (a port clash stops the earlier instance silently - not a crash)."},
			    ...
			    {"path": "/createProject", "params": "name, template=ng-objects-app|wonder-slim-app|maven, package?, location?, launch=true? (+ /launch params)", "description": "Generate a new project from a bundled template, import it into the workspace through m2e, give an application a launch configuration, and optionally launch it - from nothing to a running app in one call. package defaults to one derived from the name; location is the PARENT directory (default: the workspace directory). Refuses rather than overwrite an existing project or a non-empty directory. maven is a plain jar project for supporting logic."},
			    {"path": "/importProject", "params": "path", "description": "Import an existing Maven project directory into the workspace through m2e (no wizard). An application (build.properties names a principalClass) gets a launch configuration if it has none. Reports the project already being in the workspace instead of importing twice."},
			    {"path": "/openProject", "params": "project, related=false?", "description": "Open a closed workspace project together with its workspace dependencies (transitive, pom-resolved) - dependency resolution only sees open projects, so opening just one is rarely enough. Then clean-builds them. There is deliberately no open-everything option."},
			    {"path": "/validate", "params": "component, project?", "description": "Validate a component template and report template problems. project narrows the search to that project (falls back to every open project); found:false carries a reason - closed project, unknown project, or no such component."},
			    ...
			    {"path": "/elementApi", "params": "element (name or comma-separated list), project?, runtime=ng|wo?, raw=true?, debug=true?", "description": "The resolved binding API of one or more elements, in a project's context, as JSON: bindings with pull/push directions+types, required/default/deprecation, cross-binding constraints with their generated messages, and content/unknownAttributes policies. Names resolve through the project's tag aliases (str -> WOString -> ERXWOString); in a hybrid project (a WO app serving ng components too) runtime=ng resolves as an ng template does. raw=true returns the canonical .apiext XML instead. This is the editor hover, as data. debug=true (with project) adds a lookup trace per element: the element class found for the runtime, the root class, the raw search hits and whether each extends the root - for diagnosing class-missing markers."},
			    ...
			    {"path": "/apps", "params": "name?", "description": "Apps that self-registered (name, port, pid, runtime), liveness-checked, with the dependencies whose source is open in the workspace. name filters to one app."},
			    ...
			    {"path": "/openJavaFile", "params": "className, lineNumber?, app?", "description": "Open a Java source file in Eclipse at a line (exception-page links). app hints which project to search first."},
			    {"path": "/openComponent", "params": "component, app?, lineNumber?, offset?, length?", "description": "Open a component in the editor, optionally revealing a line or a character range (offset+length, as /validate reports them)."}
			  ]
			}""";
