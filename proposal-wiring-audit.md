...
- **Recommendation:** **feature, not cleanup.** This is *not* a one-line rewire. The original's menu used `<action class="…ActionDelegate">` entries backed by six `IEditorActionDelegate` classes that no longer exist — that whole mechanism was deliberately replaced with `org.eclipse.ui.handlers` + commands. Restoring the menu therefore means building net-new menu contributions against the modern `org.eclipse.ui.menus` extension point (command-backed, bound by `commandId` to the existing handlers), *plus* a product decision about whether the editor should carry this menu at all (the keyboard shortcuts M1+M3+1/2/3/5 already work). Track as a feature/issue, not as part of this audit's cleanup pass. The empty shell can be removed independently if we decide against the menu.
- **Confidence:** high (that it's empty; the path to populating it is feature work).
